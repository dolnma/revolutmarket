...
docker-compose -f docker-compose.prod.yml up -d nginx && docker-compose restart nginx
...
for domain in ${!domains[*]}; do
	...
	if [ -e "$data_path/conf/live/$domain_name/cert.pem" ]; then
		...
		docker-compose -f docker-compose.prod.yml run --rm --entrypoint "certbot certonly --webroot -w /var/www/letsencrypt --cert-name $domain_name $domain_args \
